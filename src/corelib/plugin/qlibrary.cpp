...
#define QT_NO_SETTINGS //plugin check without register
...
#endif
        {
            ...
            if (!pHnd) {
				...
				if (hTempModule == NULL)
				{
					...
					qDebug() << "[PluginLoad] " << "loadLibrary failed! erro:" << dwErr <<" fileName:" << fileName << __FUNCTION__ << __LINE__;
				}
...
#  endif
            ...
            if (!qtPluginQueryVerificationDataFunction
                ...
                qt_version = 0;
                key = "unknown";
				bool b = (qtPluginQueryVerificationDataFunction != 0);
				qDebug() << "[PluginLoad] " << b << qt_version << debug << key << __FUNCTION__ << __LINE__; 
                if (temporary_load)
                    unload_sys();
            } else {
                success = true;
...
#undef QT_NO_SETTINGS //plugin check without register
