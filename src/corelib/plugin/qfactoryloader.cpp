...
#define DW_QFACTORYLOADER_UPDATE

#ifdef DW_QFACTORYLOADER_UPDATE
...
void QFactoryLoader::update()
{
	...
	for (int i = 0; i < paths.count(); ++i) {
		...
		qDebug() << "[PluginLoad] " << pluginDir << __FUNCTION__ << __LINE__;
		...
		{
			qDebug() << "[PluginLoad] " << path << " doesn't exist." << __FUNCTION__ << __LINE__;
			continue;
		}
		...
		for (int j = 0; j < plugins.count(); ++j) {
			...
			if (!library->isPlugin(NULL)) {
				qDebug() << "[PluginLoad] " << library->errorString << __FUNCTION__ << __LINE__;
				library->release();
				continue;
			}
...
#endif //DW_QFACTORYLOADER_UPDATE
