...
//[YY bobdeng add]
typedef void * (_cdecl *PMALLOC)(size_t);
typedef void (_cdecl *PFREE)(void *);
typedef void * (_cdecl *PREALLOC)(void *, size_t);

static PMALLOC	s_pfnMalloc		= NULL;
static PFREE	s_pfnFree		= NULL;
static PREALLOC	s_pfnRealloc	= NULL;

extern "C" __declspec(dllexport) void setContainerAllocCallback(PMALLOC pfnMalloc, PFREE pfnFree, PREALLOC pfnRealloc)
{
	s_pfnFree = pfnFree;
	s_pfnRealloc = pfnRealloc;
	s_pfnMalloc = pfnMalloc;
}

void *qMalloc(size_t size)
{
	return s_pfnMalloc? s_pfnMalloc(size) : (::malloc(size));
}
...
void *qRealloc(void *ptr, size_t size)
{
	return s_pfnRealloc ? s_pfnRealloc(ptr, size) : (::realloc(ptr, size));
}
//[YY]
