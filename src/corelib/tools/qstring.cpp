...
QString QString::fromUtf16(const ushort *unicode, int size)
{
    ...
    const bool haveBOM = *unicode == QChar::ByteOrderSwapped || *unicode == QChar::ByteOrderMark;
    return !haveBOM ? QString((const QChar *)unicode, size) : QUtf16::convertToUnicode((const char *)unicode, size*2, 0);
}
...
QString QString::arg(const QString &a, int fieldWidth, const QChar &fillChar) const
{
	...
	if(pos != -1) {
		if(pos < (a.length() - 1)) {
			if(a.at(pos + 1).isDigit()) {
				Q_ASSERT_X(false, __FUNCTION__, "Argument contains %%number, it's a potential defect.");
				qWarning("QString::arg: Argument contains %%number: %s: ", a.toLocal8Bit().data());
			}
		}
	}
