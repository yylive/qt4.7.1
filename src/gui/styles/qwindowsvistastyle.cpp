    ...
    case CE_MenuItem:
        if (const QStyleOptionMenuItem *menuitem = qstyleoption_cast<const QStyleOptionMenuItem *>(option)) {
            ...
            if (menuitem->menuItemType == QStyleOptionMenuItem::Separator) {
                ...
                stateId = MBI_HOT;
				QRect subRect(p1.x() + (gutterWidth - menuitem->rect.x()), p1.y(), p2.x() - p1.x(), separatorSize);
            ...
            if (doTransition) {
                ...
                if (!anim) {
                    if (const QStyleOptionComboBox *combo = qstyleoption_cast<const QStyleOptionComboBox*>(option)) {
                        ...
                        startImage.fill(0);
                        QPainter startPainter(&startImage);
                        QStyleOptionSpinBox startSpinBox = *spinBox;
                        startSpinBox.state = (QStyle::State)oldState;
                        startSpinBox.activeSubControls = (QStyle::SubControl)oldActiveControls;
                        proxy()->drawComplexControl(control, &startSpinBox, &startPainter, 0 /* Intentional */);
                        t->setStartImage(startImage);
                    } else {
                        QPoint offset(0, 0);
   ...
   if (!QWindowsVistaStylePrivate::useVista())
    ...
    case CC_TitleBar:
        if (const QStyleOptionTitleBar *tb = qstyleoption_cast<const QStyleOptionTitleBar *>(option)) {
			...
			const int buttonWidth = qRound(qreal(GetSystemMetrics(SM_CXSIZE)) * QWindowsStylePrivate::nativeMetricScaleFactor(widget) - QStyleHelper::dpiScaled(4));
