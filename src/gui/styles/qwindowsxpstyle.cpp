...
#include "painting/qbackingstore_p.h"
...
static inline QWidgetBackingStore *backingStoreForWidget(const QWidget *widget)
{
	return qt_widget_private(const_cast<QWidget*>(widget))->maybeBackingStore();
}

static inline HDC hdcForWidgetBackingStore(const QWidget *widget)
{
	QWidgetBackingStore *bs = backingStoreForWidget(widget);
	if (bs)
	{
		return bs->surface()->getDC();
	}
...
QSizeF XPThemeData::size()
{
	QSizeF result(0, 0);
	if (isValid()) {
		SIZE size;
		if (SUCCEEDED(pGetThemePartSize(handle(), 0, partId, stateId, 0, TS_TRUE, &size)))
			result = QSize(size.cx, size.cy);
	}
	return result;
}

QMarginsF XPThemeData::margins(const QRect &qRect, int propId)
{
	QMarginsF result(0, 0, 0, 0);
	if (isValid()) {
		MARGINS margins;
		RECT rect = XPThemeData::toRECT(qRect);
		if (SUCCEEDED(pGetThemeMargins(handle(), 0, partId, stateId, propId, &rect, &margins)))
			result = QMargins(margins.cxLeftWidth, margins.cyTopHeight, margins.cxRightWidth, margins.cyBottomHeight);
	}
	return result;
}

QMarginsF XPThemeData::margins(int propId)
{
	QMarginsF result(0, 0, 0, 0);
	if (isValid()) {
		MARGINS margins;
		if (SUCCEEDED(pGetThemeMargins(handle(), 0, partId, stateId, propId, NULL, &margins)))
			result = QMargins(margins.cxLeftWidth, margins.cyTopHeight, margins.cxRightWidth, margins.cyBottomHeight);
	}
	return result;
}

//QSizeF XPThemeData::themeSize(const QWidget *w, QPainter *p, int themeIn, int part, int state)
//{
//	XPThemeData theme(w, p, themeIn, part, state);
//	return theme.size();
//}
//
//QMarginsF XPThemeData::themeMargins(const QRect &rect, const QWidget *w, QPainter *p, int themeIn,
...
//}
//
//QMarginsF XPThemeData::themeMargins(const QWidget *w, QPainter *p, int themeIn,
//	int part, int state, int propId)
//{
//	XPThemeData theme(w, p, themeIn, part, state);
...
const QPixmap *QWindowsXPStylePrivate::tabBody(QWidget *widget)
{
    if (!tabbody) {
		...
		const QSize sz = (theme.size() * QWindowsStylePrivate::nativeMetricScaleFactor(widget)).toSize();

        tabbody = new QPixmap(sz.width(), QApplication::desktop()->screenGeometry().height());
        QPainter painter(tabbody);
        theme.rect = QRect(0, 0, sz.width(), sz.height());
        drawBackground(theme);
        // We fill with the last line of the themedata, that
        // way we don't get a tiled pixmap inside big tabs
        QPixmap temp(sz.width(), 1);
        painter.drawPixmap(0, 0, temp, 0, sz.height() -1, -1, -1);
        painter.drawTiledPixmap(0, sz.height(), sz.width(), tabbody->height()-sz.height(), temp);
    }
    return tabbody;
}
...
void QWindowsXPStylePrivate::drawBackground(XPThemeData &themeData)
{
	...
	if (themeData.widget && painter->opacity() == 1.0 && !themeData.rotate
		...
		case QInternal::Image:
			...
			if (QWidgetBackingStore  *bs = backingStoreForWidget(themeData.widget))
				if (bs->surface()->geometry().size().isValid() && bs->surface()->paintDevice() == enginePaintDevice)
					canDrawDirectly = true;
		}
	}
    ...
    case CE_SizeGrip:
        {
			...
			QSize sz = (theme.size() * QWindowsStylePrivate::nativeMetricScaleFactor(widget)).toSize();
			sz.rheight()--;

            if (const QStyleOptionSizeGrip *sg = qstyleoption_cast<const QStyleOptionSizeGrip *>(option)) {
                switch (sg->corner) {
                    case Qt::BottomRightCorner:
                        rect = QRect(rect.right() - sz.width(), rect.bottom() - sz.height(), sz.width(), sz.height());
                        break;
                    case Qt::BottomLeftCorner:
                        rect = QRect(rect.left() + 1, rect.bottom() - sz.height(), sz.width(), sz.height());
                        hMirrored = true;
                        break;
                    case Qt::TopRightCorner:
                        rect = QRect(rect.right() - sz.width(), rect.top() + 1, sz.width(), sz.height());
                        vMirrored = true;
                        break;
                    case Qt::TopLeftCorner:
                        rect = QRect(rect.left() + 1, rect.top() + 1, sz.width(), sz.height());
        ...
        if (qstyleoption_cast<const QStyleOptionDockWidget *>(option))
        {
            if (d->dockFloat.isNull()) {
                ...
                if (theme.isValid()) {
                    ...
                    QPixmap pm = QPixmap(size.width(), size.height());
                    pm.fill(Qt::transparent);
                    QPainter p(&pm);
                    theme.painter = &p;
                    theme.rect = QRect(0, 0, size.width(), size.height());
                    d->drawBackground(theme);
