...
void QGraphicsViewPrivate::mouseMoveEventHandler(QMouseEvent *event)
{
    ...
    // [/YY Comment]
    // [YY Add]
    if (mouseEvent.isAccepted() && (mouseEvent.buttons() & (Qt::LeftButton | Qt::RightButton)) != 0) {
    // [/YY Add]
...
void QGraphicsView::paintEvent(QPaintEvent *event)
{
	...
	//if (QHighDpiScaling::isActive() && qCeil(QHighDpiScaling::factor(this)) > QHighDpiScaling::factor(this))
	//{
    ...
    if ((d->cacheMode & CacheBackground)
        ...
        if (d->mustResizeBackgroundPixmap) {
			// viewport()->size() is empty (width = 0 or height = 0) -> backgroundPixmap is invalid
			// -> backgroundPainter is invalid -> drawBackground maybe crash
