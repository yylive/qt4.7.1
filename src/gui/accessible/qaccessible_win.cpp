...
HRESULT STDMETHODCALLTYPE QWindowsAccessible::put_accName(VARIANT varID, BSTR szValue)
{
    showDebug(__FUNCTION__, accessible);
	//return DISP_E_MEMBERNOTFOUND; 
	/*  Qt 只实现了MSAA协议的读接口， 所有写接口全部直接返回DISP_E_MEMBERNOTFOUND，
		导致无法调用QAcessible::setValue()等方法 。 
		*/
	if (!accessible->isValid()) {
        return E_FAIL;
	}
    accessible->setText(Name, varID.lVal, QString::fromWCharArray(_bstr_t(szValue)));
    return S_OK;
}
...
HRESULT STDMETHODCALLTYPE QWindowsAccessible::put_accValue(VARIANT varID, BSTR szValue)
{
    showDebug(__FUNCTION__, accessible);
    //return DISP_E_MEMBERNOTFOUND; 
	/*  Qt 只实现了MSAA协议的读接口， 所有写接口全部直接返回DISP_E_MEMBERNOTFOUND，
    ...
    accessible->setText(Value, varID.lVal, QString::fromWCharArray(_bstr_t(szValue)));
    return S_OK;
}
