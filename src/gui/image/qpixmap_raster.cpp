...
int depthForFormat(QImage::Format format);
    ...
    if (image.depth() == 1) {
        ...
        if (alpha != 255) {
            if (!image.hasAlphaChannel()) {
				...
				if (!image.isNull() && depthForFormat(image.format()) == depthForFormat(toFormat)) {
					image.detach();
...
QImage QRasterPixmapData::toImage(const QRect &rect) const
{
	...
	if (d % 8 == 0 && (((uint(clipped.x()) * d)) % 32 == 0)){
...
void QRasterPixmapData::createPixmapForImage(QImage &sourceImage, Qt::ImageConversionFlags flags, bool inPlace)
{
    ...
    if (inPlace && sourceImage.d && sourceImage.d->convertInPlace(format, flags)) {
        image = sourceImage;
    } else {
...
qreal QRasterPixmapData::devicePixelRatio() const
{
	return image.devicePixelRatio();
}

void QRasterPixmapData::setDevicePixelRatio(qreal scaleFactor)
{
