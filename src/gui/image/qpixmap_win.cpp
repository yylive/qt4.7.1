...
QPixmap QPixmap::fromWinHBITMAP(HBITMAP bitmap, HBitmapFormat format)
{
    ...
    // Get bitmap bits
    // [YY.Add]
    // GDI function require entire buffer from single VirtualAlloc
    // some memory allocator may break this rule and cause a GetDIBits failures
    // use HeapAlloc/HeapFree can avoid this error
    // qt_fromWinHBITMAP has same problem, we modified it too
    uchar *data = (uchar *)HeapAlloc(GetProcessHeap(), 0, bmi.bmiHeader.biSizeImage);
