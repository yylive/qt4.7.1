...
void QTextLayout::drawCursor(QPainter *p, const QPointF &pos, int cursorPosition, int width) const
{
	...
	qreal cursorHeight = sl.base().toReal();
    if ( d->layoutData->items.size() > 0 )
	{
		QFontMetricsF fm(d->font());
		if ( fm.height() > cursorHeight )
		{
			cursorHeight = fm.height();//pointHeight;
		}
		int count = d->layoutData->items.size();
		for ( int i = 0; i < count; ++i)
		{
			const QScriptItem &si = d->layoutData->items.at(i);
			int pointHeight = d->font(si).pointSize();
			if (pointHeight > 0)
			{			
				if (si.ascent > 0)
				{
        		...
        		rightToLeft = si.analysis.bidiLevel % 2;
				isItalics = d->font(si).italic();
				QFontMetricsF fm(d->font(si));
				if ( fm.height() < cursorHeight )
				{
					cursorHeight = fm.height();//pointHeight;
				}
	...
	//[YY.Add]
	qreal bottomLine = y + (base + descent + 1).toReal();
	qreal cursorY = bottomLine - cursorHeight;
	
	if ( isItalics )
	{
		int xOffset = qTan( 3.1415 / 10.0 ) * cursorHeight;
		p->drawLine(QPointF(x + xOffset,cursorY), QPointF( x, cursorY + cursorHeight) );
	}
	else
	{
		p->fillRect(QRectF(x, cursorY, qreal(width), cursorHeight), p->pen().brush());
	}
    //[/YY]
...
{
	//return eng->lines[0].height().toReal();
...
void QTextLine::layout_helper(int maxGlyphs)
{
    ...
    while (newItem < eng->layoutData->items.size()) {
        ...
        //[YY.comment]
		// To enable using the space for wrap
...
found:
	...
	int from = line.from;
	int lineEnd = from + line.length;

	QFixed newAscent = 0;
	QFixed newDescent = 0;
	for(int index = 0; index < eng->layoutData->items.size(); index++)
 	{
 		QScriptItem &si = eng->layoutData->items[index];
		int pos = si.position;
		int end = pos + eng->length(index);
		
		if ( from < end && lineEnd > pos )
		{	
			newAscent = qMax(newAscent, si.ascent);
			newDescent = qMax(newDescent, si.descent);
		}
 	}

	if (newAscent > 0 && newDescent > 0)
	{
		line.ascent = newAscent;
		line.descent = newDescent;
		//[YY.comment]
		//Increase the distance of the Line spacing(for dotline)
  		if ( eng->layoutData )
  		{
  			int len = eng->layoutData->items.size();
  			for(int j = 0; j < len; ++j)
  			{
  				const QScriptItem &si = eng->layoutData->items[j];
  				const QTextCharFormat &f = eng->format(&si);
  				if ( QTextCharFormat::DotLine == f.underlineStyle() )
  				{
  					line.descent += QFixed(2);
  					break;
  				}
  			}
  		}
...
//     qDebug() << "right of last";
    ...
    //[YY.comment]
	//to fix the problem that can't select the last char with mouse.
