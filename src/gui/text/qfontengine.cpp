        ...
        if (!transform) {
            while (i < glyphs.numGlyphs) {
                if (!glyphs.attributes[i].dontPrint) {
					...
					offsetX = glyphs.offsets[i].x;
					offsetY = glyphs.offsets[i].y;
					if ( offsetY < 0 )
					{
						offsetY = 0;
					}
					...
					positions[current].x = xpos + offsetX;
					positions[current].y = ypos + offsetY;
					//[/YY]
...
//[YY. zyf add]
#define GLYPH_CACHE_INVALID_TIME (60 * 5)
void QFontEngine::tryClearGlyphCache()
{
	QTime currentTime = QTime::currentTime();
	for (QLinkedList<GlyphCacheEntry>::iterator it = m_glyphCaches.begin();
		it != m_glyphCaches.end(); )
	{
		QFontEngineGlyphCache *c = it->cache;
		if (c->m_timeStamp.secsTo(currentTime) >= GLYPH_CACHE_INVALID_TIME)
		{
			delete c;
...
QFontEngineGlyphCache *QFontEngine::glyphCache(void *key, QFontEngineGlyphCache::Type type, const QTransform &transform) const
{
    for (QLinkedList<GlyphCacheEntry>::const_iterator it = m_glyphCaches.constBegin(), end = m_glyphCaches.constEnd(); it != end; ++it) {
        ...
        if (key == it->context
			...
			//[YY. zyf add]
			c->m_timeStamp = QTime::currentTime();
			//[YY]
            return c;
        }
    }
...
//[YY. zyf add]
void QFontEngineMulti::tryClearGlyphCache()
{
	QFontEngine::tryClearGlyphCache();
	for (int i = 0; i < engines.size(); ++i)
	{
		QFontEngine* fontEngine = engines.at(i);
		if (fontEngine)
			fontEngine->tryClearGlyphCache();
	}
}
//[YY]
