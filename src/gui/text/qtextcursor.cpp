...
*/
void QTextCursor::insertText(const QString &string, const QTextCharFormat &_format)
{
	QString text = string;
    ...
    if (!text.isEmpty()) {
		...
		//[YY. Add]
		bool fixup = false;
		d->priv->aboutToInsertText(text, format, fixup);
		if ( fixup && text.isEmpty() )
		{
			return;
		}
		//[/YY]
...
void QTextCursor::insertBlock(const QTextBlockFormat &format, const QTextCharFormat &_charFormat)
{
	...
	//[YY.Add]
	bool fixup = false;
	d->priv->aboutToInsertText( QString(QChar::ParagraphSeparator), _charFormat, fixup); 
	if (fixup) 
	{
		return ;
	}
