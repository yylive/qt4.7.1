...
//[YY. zyf add]
template <class T, int DEFVAL>
class QWidthCacheImpl
{
public:
	QWidthCacheImpl<T, DEFVAL>() : widthCache(NULL), widthCacheSize(0) 
    {
	}
	~QWidthCacheImpl<T, DEFVAL>()
	{
		if (widthCache)
			free(widthCache);
	}
...
public:
	T getGlyphWidth(unsigned int glyph, bool& found) const
	{
		if(int(glyph) >= widthCacheSize) {
			int newSize = (glyph + 256) >> 8 << 8;
			widthCache = q_check_ptr((T*)realloc(widthCache,
				newSize*sizeof(T)));
			for(int i = widthCacheSize; i < newSize; ++i)
				widthCache[i] = T(DEFVAL);
			widthCacheSize = newSize;
		}
		found = (widthCache[glyph] != T(DEFVAL));
		return widthCache[glyph];
	}
	void setGlyphWidth(unsigned int glyph, int width)
	{
		Q_ASSERT(glyph < widthCacheSize);
		widthCache[glyph] = (T)width;
	}
	
private:
	mutable T* widthCache;
	mutable uint widthCacheSize;
};

template <class T, int DEFVAL>
class QWidthCacheImplEx
{
	enum { MAX_CACHE_TABLE_CNT = 1 << 8, };
	enum { MAX_CACHE_TABLE_OFFSET = 1 << 8, };
...
#pragma pack(push, 1)
	struct GlyphIndex
	{
		union
		{
			ushort simpleIndexVal;
			struct 
			{
#if Q_BYTE_ORDER == Q_LITTLE_ENDIAN
				ushort tableOffset : 8;
				ushort tableIndex : 8;
#else
				ushort tableIndex : 8;
				ushort tableOffset : 8;
	...
	struct GlyphCacheTrunk
	{
		T cache[MAX_CACHE_TABLE_OFFSET];
	};
	struct GlyphCacheTable
	{
		ushort index[MAX_CACHE_TABLE_CNT];
	};
#pragma pack(pop)

public:
	QWidthCacheImplEx<T, DEFVAL>() : m_cacheTable(NULL)
	{
	}

	~QWidthCacheImplEx<T, DEFVAL> ()
	{
		if (m_cacheTable)
			delete m_cacheTable;
		foreach(GlyphCacheTrunk* trunk, m_cacheTrunks)
			delete trunk;	
	}
...
public:
	T getGlyphWidth(unsigned int glyph, bool& found) const
	{
		if (glyph >= 65536) return DEFVAL;//对于大于65536的图元索引，应该极其罕见，我们不缓存。
		GlyphIndex gIndex = { glyph };
		GlyphCacheTrunk* trunk = _getGlyphCacheTrunk(gIndex.complexIndexVal.tableIndex);
		found = (trunk->cache[gIndex.complexIndexVal.tableOffset] != T(DEFVAL));
		return trunk->cache[gIndex.complexIndexVal.tableOffset];
	}
	void setGlyphWidth(unsigned int glyph, int width)
	{
		if (glyph >= 65536) return;//对于大于65536的图元索引，应该极其罕见，我们不缓存。
		GlyphIndex gIndex = { glyph };
		GlyphCacheTrunk* trunk = _getGlyphCacheTrunk(gIndex.complexIndexVal.tableIndex);
		trunk->cache[gIndex.complexIndexVal.tableOffset] = (T)width;
	}

private:
	GlyphCacheTrunk* _getGlyphCacheTrunk(ushort tableIndex) const
	{
		Q_ASSERT(tableIndex < MAX_CACHE_TABLE_CNT);
		//做一点优化：小于256个图元的字体（绝大多数的英文字体），不再需要附加的索引表
		if (tableIndex == 0 && m_cacheTable == NULL)
		{
			if (m_cacheTrunks.empty())
				m_cacheTrunks.push_back(__createGlyphCacheTrunk());
			return m_cacheTrunks[0];
		}
		
		if (m_cacheTable == NULL)//确保索引表被创建
		{
			m_cacheTable = __createGlyphCacheTable();
			if (!m_cacheTrunks.empty())
				m_cacheTable->index[0] = 0;
		}

		if (m_cacheTable->index[tableIndex] == MAX_CACHE_TABLE_OFFSET)//确保Trunk被创建
		{
			m_cacheTrunks.push_back(__createGlyphCacheTrunk());
			m_cacheTable->index[tableIndex] = m_cacheTrunks.size() - 1;
		}

		Q_ASSERT(m_cacheTable->index[tableIndex] < m_cacheTrunks.size());
		return m_cacheTrunks[(m_cacheTable->index[tableIndex])];
	}

private:
	static GlyphCacheTrunk* __createGlyphCacheTrunk()
	{
		 GlyphCacheTrunk* trunk = new GlyphCacheTrunk;
		 for (int i = 0; i < MAX_CACHE_TABLE_OFFSET; ++i)
			 trunk->cache[i] = T(DEFVAL);
		 return trunk;
	}
	static GlyphCacheTable* __createGlyphCacheTable()
	{
		GlyphCacheTable* cacheTable = new GlyphCacheTable;
		for (int i = 0; i < MAX_CACHE_TABLE_CNT; ++i)
			cacheTable->index[i] = MAX_CACHE_TABLE_OFFSET;
		return cacheTable;
	}

private:
	mutable QVector<GlyphCacheTrunk*> m_cacheTrunks;
	mutable GlyphCacheTable* m_cacheTable;
};
// [YY]
...
#else //[YY. zyf add]
	widthCache = new QWidthCacheImplEx<unsigned char, 0>();
	designAdvances = new QWidthCacheImplEx<QFixed, -1000000>();
#endif //[YY]
...
#else //[YY. zyf add]
			bool found = false;
			QFixed advance = designAdvances->getGlyphWidth(glyph, found);
			if (!found)
			{
				if (!oldFont)
					oldFont = selectDesignFont();
				...
				int width = 0;
				calculateTTFGlyphWidth(hdc, glyph, width);
				advance = QFixed(width) / designToDevice;
				designAdvances->setGlyphWidth(glyph, advance.value());
			}
...
#else //[YY. zyf add]
			bool found = false;
			int width = widthCache->getGlyphWidth(glyph, found);
			if (!found)
			{
				if (!oldFont)
					oldFont = SelectObject(hdc, hfont);
				...
				// if glyph's within cache range, store it for later
				if (width > 0 && width < 0x100)
					widthCache->setGlyphWidth(glyph, width);
			}
