...
static bool defaultLoadAllFonts = true;
...
static void initializeDb()
{
    ...
    //font list init optimization
    if(defaultLoadAllFonts)
...
} fontConfigs[QUnicodeTables::ScriptCount + 1];
...
static QFontEngine *loadWin(const QFontPrivate *d, int script, const QFontDef &req)
{
    ...
    family_list << familyList(QApplication::font().family());

    // append the default fallback font for the specified script
    // family_list << ... ; ###########
    const int boundedScript = qBound<int>(QUnicodeTables::Common, script, QUnicodeTables::ScriptCount);
    family_list << fontConfigs[boundedScript].fallbacks;
    ...
    {
        QStringList distinct_family_list;
        const QStringList &blacklist = fontConfigs[boundedScript].blacklist;
        foreach(const QString &family, family_list)
        {
            const QString englishName = getEnglishName(family);
            if (!distinct_family_list.contains(family) && !blacklist.contains(family))
                distinct_family_list << family;
        }
        qSwap(family_list, distinct_family_list);
    }
...
bool QFontDatabase::removeApplicationFont(int handle)
{
    ...
    QFontDatabasePrivate *db = privateDb();
    // Magic number to clear all loaded fonts
    if (handle == -1)
        db->invalidate();
...
Q_GUI_EXPORT void setDefaultLoadAllFonts(const bool b)
{
    defaultLoadAllFonts = b;
}

Q_GUI_EXPORT void setDefaultFontConfig(const QStringList &fallbacks, const QStringList &blacklist)
{
    {
        int count = qMin(fallbacks.count(), QUnicodeTables::ScriptCount - 1);
        for (int i = 0; i < count; ++i)
        {
            fontConfigs[i].fallbacks = familyList(fallbacks[i]);
        }
    }
    {
        int count = qMin(blacklist.count(), QUnicodeTables::ScriptCount - 1);
        for (int i = 0; i < count; ++i)
        {
            fontConfigs[i].blacklist = familyList(blacklist[i]);
        }
    }
}
