...
class QNativeImage;
//[YY. zyf add]
template <class T, int DEFVAL> class QWidthCacheImplEx;
//[YY]
...
#else //[YY. zyf add]
	mutable QWidthCacheImplEx<unsigned char, 0>* widthCache;
	mutable QWidthCacheImplEx<QFixed, -1000000>* designAdvances;
#endif //[YY]
...
class QFontEngineWinCache : public QObject
{
public:
	static QFontEngineWinCache *instance();
	
	QFontEngineWinCache();
	~QFontEngineWinCache();

public:
	QFontEngine* getFontEngineWin(const QString& name, HFONT font, bool stockFont, LOGFONT lf); 
	bool removeFontEngineWin(QFontEngine* fontEngine);

private:
	struct Key {
		Key() : stockFont(false) { ::memset(&lf, 0, sizeof(lf)); };
		Key(const QString& _name, bool _stockFont, LOGFONT _lf)
			: name(_name), stockFont(_stockFont), lf(_lf) {};

		inline bool operator<(const Key& other) const
		{
			if (name != other.name) return name < other.name;
			if (stockFont != other.stockFont) return stockFont < other.stockFont;
			return ::memcmp(&lf, &other.lf, sizeof(lf) - sizeof(lf.lfFaceName));
		}

		inline bool operator==(const Key& other) const
		{
			return name == other.name && stockFont == other.stockFont 
				&& ::memcmp(&lf, &other.lf, sizeof(lf) - sizeof(lf.lfFaceName)) == 0;
		}
	...
	typedef QMap<Key,QFontEngineWin*> FontEngineWinCache;
	FontEngineWinCache engineCache;
};
