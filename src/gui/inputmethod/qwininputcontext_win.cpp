...
LRESULT QWinInputContext::DefWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        // [YY.Modify]
        // We process WM_INPUTLANGCHANGEREQUEST(0x50) ourselves.
        // Sometimes DefWindowProc blocks WM_INPUTLANGCHANGEREQUEST for a long time.
        if (WM_INPUTLANGCHANGEREQUEST == msg)
            return (::ActivateKeyboardLayout((HKL)lParam, KLF_REPLACELANG) != NULL);
        // [YY.End]
...
void QWinInputContext::update()
{
	...
	if (QHighDpiScaling::isActive())
	{
		 if (f.pixelSize() > 0)
		{
			f.setPixelSize(QHighDpi::toNativePixels(f.pixelSize(), w));
		}
		else if (f.pointSizeF() > 0)
		 {
			f.setPointSizeF(QHighDpi::toNativePixels(f.pointSizeF(), w));	
		 }
	}
	...
	r = QHighDpi::toNativePixels(r, imeWnd);
	...
	cf.ptCurrentPos.y = r.y() + 3;
	//[YY.Modify]
	//cf.rcArea.left = r.x();
	//cf.rcArea.top = r.y();
	//cf.rcArea.right = r.x() + r.width();
	//cf.rcArea.bottom = r.y() + r.height();
	//[YY.End]
    CANDIDATEFORM candf;
    candf.dwIndex = 0;
