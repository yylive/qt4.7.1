...
HDC QRasterWindowSurface::getDC() const
{
	return d_ptr->image->hdc;
}
...
void QRasterWindowSurface::flush(QWidget *widget, const QRegion &rgn, const QPoint &offset)
{
	...
	QRegion native_rgn = QHighDpi::toNativeLocalRegion(rgn, widget);
	QPoint native_offset = QHighDpi::toNativeLocalPosition(offset, widget);

#ifdef Q_WS_WIN
    QRect br = native_rgn.boundingRect();
    ...
    {
        QRect r = QHighDpi::toNativePixels(window()->frameGeometry(), widget) ;
        QPoint frameOffset = QHighDpi::toNativePixels(qt_widget_private(window())->frameStrut().topLeft(), window());
        QRect dirtyRect = br.translated(native_offset + frameOffset);
		...
		const BOOL result = ptrUpdateLayeredWindowIndirect(window()->internalWinId(), &info);
		if (!result)
...
#endif
    {
		...
		wOffset = QHighDpi::toNativePixels(wOffset, widget);
	...
	if (0) {
		static int n = 0;
		const QString fileName = QString::fromLatin1("d:/temp/dpi/win%1_%2.png").
			arg((long)window()->winId()).arg(n++);
		d->image->image.save(fileName);
	}
...
void QRasterWindowSurface::setGeometry(const QRect &rect)
{
	...
	QSize native_size = QHighDpi::toNativePixels(rect.size(), window());

    d->inSetGeometry = true;
    if (d->image == 0 || d->image->width() < native_size.width() || d->image->height() < native_size.height()) {
#if (defined(Q_WS_X11) && !defined(QT_NO_XRENDER)) || (defined(Q_WS_WIN) && !defined(Q_WS_WINCE))
#ifndef Q_WS_WIN
...
bool QRasterWindowSurface::scroll(const QRegion &area, int dx, int dy)
{
	...
	const qreal nativeDx = QHighDpi::toNativePixels(qreal(dx), window());
	const qreal nativeDy = QHighDpi::toNativePixels(qreal(dy), window());
	if (qFloor(nativeDx) != nativeDx || qFloor(nativeDy) != nativeDy)
		return false;

    QRect rect = QHighDpi::toNativeLocalRegion(area, window()).boundingRect();
    BitBlt(d->image->hdc, rect.x()+ nativeDx, rect.y()+ nativeDy, rect.width(), rect.height(),
...
void QRasterWindowSurface::prepareBuffer(QImage::Format format, QWidget *widget)
{
	...
	QSize s = QHighDpi::toNativePixels(QSize(width, height), widget);
	width = s.width();
	height = s.height();
	...
	d->image->image.setDevicePixelRatio(widget->devicePixelRatioF());
    ...
    if (oldImage && d->inSetGeometry && hasStaticContents()) {
		...
		staticRegion = QHighDpi::toNativeLocalRegion(staticRegion, widget);
