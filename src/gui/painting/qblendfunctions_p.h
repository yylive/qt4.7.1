...
//              << " - sx=" << sx << " sy=" << sy << " ix=" << ix << " iy=" << iy;
	...
	// [YY.Add] For QTBUG-9340 : Access violation when drawing scaled images with certain dimensions
	// https://bugreports.qt-project.org/browse/QTBUG-9340
	const quint32 sw = qFloor( srcRect.right() );
	const quint32 sh = qFloor( srcRect.bottom() );
	if (((srcy + (h - 1) * iy) >> 16) >= sh)
	{
		--h;
	}
	if (((basex + (w - 1) * ix) >> 16) >= sw)
	{
		--w;
	}
	// [/YY.End]
