...
//                positions[i].x.toInt(), positions[i].y.toInt());
	...
	QTextureGlyphCache *cache = 
		static_cast<QTextureGlyphCache*>(fontEngine->glyphCache(0, glyphType, s->matrix));
	if (!cache || cache->reachCacheBand()) {
		cache = QTextureGlyphCache::createGlyphCache(glyphType, s->matrix);
		fontEngine->setGlyphCache(0, cache);
	}
	...
	for (int i=0; i<numGlyphs; ++i)
	{
		...
		int bpl = 0, depth = 0, rx = 0, ry = 0, w = 0, h = 0;
		if (cache->getGlyphCacheData(glyphs[i], positions + i, &data, &bpl, &depth, &rx, &ry, &w, &h))
			alphaPenBlt(data, bpl, depth, rx, ry, w, h);
	}
