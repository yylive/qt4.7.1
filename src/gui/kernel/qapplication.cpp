...
QPointer<QWidget> QApplicationPrivate::active_window = 0;        // toplevel with keyboard focus
                                                ...
                                                QWidget *alienWidget)
{
    ...
    // [/YY Comment]
    // [YY Add]
    if (((type == QEvent::MouseMove && (buttons & (Qt::LeftButton | Qt::RightButton))) || (type == QEvent::MouseButtonRelease))
    // [/YY Add]
                                         ...
                                         bool spontaneous)
{
    ...
    if (*buttonDown) {
        if (!graphicsWidget) {
            ...
            //[/YY comment]
            //[YY add]
            if (event->type() == QEvent::MouseButtonRelease && 0 == (event->buttons() & (Qt::LeftButton | Qt::RightButton)))
            //[/YY add]
                *buttonDown = 0;
        }
...
QPixmap QApplicationPrivate::getPixmapCursor(Qt::CursorShape cshape)
{
    ...
    if (!move_cursor) {
		...
		if (QHighDpiScaling::isActive())
		{
			...
			pixs << move_cursor << copy_cursor << link_cursor << ignore_cursor;
			foreach(QPixmap* pix, pixs)
			{
				*pix = pix->scaled(QHighDpi::toNativePixels(pix->size(), (QWidget*)NULL), Qt::KeepAspectRatio, Qt::SmoothTransformation );
			}
		}
