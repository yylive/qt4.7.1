...
QPoint QCursor::pos()
{
    POINT p;
    GetCursorPos(&p);
	return QHighDpi::fromNativePixels(QPoint(p.x, p.y), QApplicationPrivate::platform_integration->primaryScreen());
    //return QPoint(p.x, p.y);
}

void QCursor::setPos(int x, int y)
{
	QPoint pos = QHighDpi::toNativePixels(QPoint(x, y), QApplicationPrivate::platform_integration->primaryScreen());
...
void QCursorData::update()
{
    ...
    if (cshape == Qt::BitmapCursor && !pixmap.isNull()) {
		//QPixmap p = pixmap;
		//const qreal pixmapScaleFactor = QHighDpiScaling::factor((QWidget*)0) / pixmap.devicePixelRatioF();
		//if (!qFuzzyCompare(pixmapScaleFactor, 1)) {
		//	p = p.scaled((pixmapScaleFactor * QSizeF(pixmap.size())).toSize(),
		//		Qt::KeepAspectRatio, Qt::SmoothTransformation);
		//}

		//QPoint hotSpot = QPoint(hx, hy);
		//hotSpot = QHighDpi::toNativePixels(hotSpot, (QWidget*)NULL);
  //      hcurs = create32BitCursor(p, hotSpot.x(), hotSpot.y());
   ...
   static const uchar openhand_bits[] = {
    ...
    case Qt::BitmapCursor: {
		...
		{
			QSize sacledSize = QHighDpi::toNativePixels(bbits.size(), 0);
			//图标宽度除以8值要为双数，要处理一下.
			int n = sacledSize.width() / 8;
			if ( 0 != ( n % 2))
			{
				int nWidth = (n + 1) * 8;
				int nHeight = nWidth * sacledSize.height() / sacledSize.width();
				sacledSize = QSize(nWidth, nHeight);
			}
			bbits = bbits.scaled(sacledSize);
			mbits = mbits.scaled(sacledSize);
		}
		...
		QPoint hotSpot = QPoint(hx, hy);
		hotSpot = QHighDpi::toNativePixels(hotSpot, (QWidget*)NULL);
        hcurs = CreateCursor(qWinAppInst(), hotSpot.x(), hotSpot.y(), bbits.width(), bbits.height(),
                                   xBits, xMask);
        delete [] xBits;
        delete [] xMask;
    ...
    case Qt::DragLinkCursor: {
        ...
        hcurs = create32BitCursor(pixmap, hotSpot.x(), hotSpot.y());
    }
        return;
    default:
