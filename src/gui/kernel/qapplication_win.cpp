...
void qt_init(QApplicationPrivate *priv, int)
{
  ...
  // Get command line params
    ...
    // It should be set in manifest. huangjianxiong 2014.12.24
...
void QApplicationPrivate::createEventDispatcher()
{
	...
	QApplicationPrivate::platform_integration = new QPlatformIntegration();
	QApplicationPrivate::platform_integration->updateDpiSetting();
...
QWidget *QApplication::topLevelAt(const QPoint &pos)
{
    ...
    HWND  win;
    QWidget *w;
	const QPoint native_pos = QHighDpi::toNativePixels(pos, QApplicationPrivate::platform_integration->primaryScreen());
    p.x = native_pos.x();
    p.y = native_pos.y();
    ...
    */
    ...
    if (qt_is_translatable_mouse_event(message)) {
        if (QApplication::activePopupWidget() != 0) { // in popup mode
            //POINT curPos = msg.pt;
			const QPoint curPos = QHighDpi::fromNativePixels(QPoint(msg.pt.x, msg.pt.y), (QWidget*)NULL);
            QWidget* w = QApplication::widgetAt(curPos);
            if (w)
                widget = (QETWidget*)w;
        }
        ...
        case WM_WINDOWPOSCHANGING:
            {
                ...
                if (widget->isWindow()) {
                    ...
                    if (widget->layout() && widget->layout()->hasHeightForWidth()
						...
						QRect logicWinRect = QHighDpi::fromNativePixels(QRect(winPos->x, winPos->y, winPos->cx, winPos->cy), widget);
                        QRect newRect = QRect(logicWinRect.left() + fs.left(),
                                              logicWinRect.top() + fs.top(),
											  logicWinRect.width() - fs.left() - fs.right(),
											  logicWinRect.height() - fs.top() - fs.bottom());
						...
						newRect = QHighDpi::toNativePixels(newRect, widget);
        ...
        case WM_GETMINMAXINFO:
            if (widget->xtra()) {
                ...
                QRect fs = widget->frameStrut();
                if ( x->minw > 0 )
                    mmi->ptMinTrackSize.x = QHighDpi::toNativePixels(x->minw, widget)  + fs.right() + fs.left();
                if ( x->minh > 0 )
                    mmi->ptMinTrackSize.y = QHighDpi::toNativePixels(x->minh, widget) + fs.top() + fs.bottom();
                qint32 maxw = (x->maxw >= x->minw) ? x->maxw : x->minw;
                qint32 maxh = (x->maxh >= x->minh) ? x->maxh : x->minh;
                if ( maxw < QWIDGETSIZE_MAX ) {
                    mmi->ptMaxTrackSize.x = QHighDpi::toNativePixels(maxw, widget) + fs.right() + fs.left();
                ...
                if ( maxh < QWIDGETSIZE_MAX )
                    mmi->ptMaxTrackSize.y = QHighDpi::toNativePixels(maxh, widget) + fs.top() + fs.bottom();
                RETURN(0);
            }
            break;
...
void QApplicationPrivate::closePopup(QWidget *popup)
{
    ...
    if (QApplicationPrivate::popupWidgets->isEmpty()) { // this was the last popup
        ...
        replayPopupMouseEvent = (!popup->geometry().contains(curPos)
...
#define PEEKMESSAGE_IS_BROKEN 1
    ...
    const QPoint widgetPos = mapFromGlobal(QHighDpi::fromNativePixels(QPoint(msg.pt.x, msg.pt.y), this));
    ...
    if (type == QEvent::MouseMove || type == QEvent::NonClientAreaMouseMove
        ...
        pos.rx() = curPos.x;
        pos.ry() = curPos.y;
		pos = QHighDpi::fromNativePixels(pos, this);
        pos = d_func()->mapFromWS(pos);
    } else {
        gpos = msg.pt;
        pos = mapFromGlobal(QHighDpi::fromNativePixels(QPoint(gpos.x, gpos.y), this));
    ...
    if (qApp->d_func()->inPopupMode()) {                        // in popup mode
        ...
        if (target != this) {
            if ((windowType() == Qt::Popup) && rect().contains(pos) && 0)
                ...
                pos = target->mapFromGlobal(QHighDpi::fromNativePixels(globalPos, this));
        }
        ...
        if (target->isEnabled()) {
            ...
            pos = target->mapFromGlobal(QHighDpi::fromNativePixels(globalPos, this));
                QMouseEvent e(type, pos, QHighDpi::fromNativePixels(globalPos, this),
        ...
        //[YY add]
        bool allowCapture = ((bs & Qt::LeftButton) || (bs & Qt::RightButton));
        bool allowRelease = (0 == (bs & (Qt::LeftButton | Qt::RightButton)));
        //[/YY add]
        ...
        //[YY add]
        if ((type == QEvent::MouseButtonPress ||
              type == QEvent::MouseButtonDblClick) && allowCapture) {
        //[/YY add]
            Q_ASSERT(testAttribute(Qt::WA_WState_Created));
            if (QWidget::mouseGrabber() == 0)
        ...
        //[/YY comment]
        //[YY add]
        } else if (type == QEvent::MouseButtonRelease && allowRelease) {
        //[/YY add]
            if (QWidget::mouseGrabber() == 0)
                releaseAutoCapture();
        }

        const QPoint globalPos = QHighDpi::fromNativePixels(QPoint(gpos.x, gpos.y), this);
...
bool QETWidget::translateWheelEvent(const MSG &msg)
{
	...
	globalPos = QHighDpi::fromNativePixels(globalPos, this);
...
bool QETWidget::translatePaintEvent(const MSG &msg)
{
	...
	const QRect updateRect = QRect(QPoint(ps.rcPaint.left, ps.rcPaint.top), QPoint(ps.rcPaint.right, ps.rcPaint.bottom));

    // Mapping region from system to qt (32 bit) coordinate system.
	//qDebug() << "wm_paint updateRect=" << updateRect << ", wrect="<< data->wrect << ", ret=" << updateRect.translated(data->wrect.topLeft());
    d_func()->syncBackingStore(QHighDpi::fromNativeLocalExposedRegion(updateRect, this).translated(data->wrect.topLeft()));
...
bool QETWidget::translateConfigEvent(const MSG &msg)
{
    ...
    if (msg.message == WM_SIZE) {                // resize event
        ...
        QSize oldSize = size();
        QSize newSize(a, b);
		newSize = QHighDpi::fromNativePixels(newSize, this);
#ifdef Q_WS_WINCE_WM
        ...
        QPoint newCPos(a, b);
		newCPos = QHighDpi::fromNativePixels(newCPos, this);
