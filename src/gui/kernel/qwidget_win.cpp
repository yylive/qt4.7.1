...
void QWidgetPrivate::create_sys(WId window, bool initializeWindow, bool destroyOldWindow)
{
    ...
    if (sw < 0) {                                // get the (primary) screen size
		...
		QSize size(sw, sh);
		size = QHighDpi::fromNativePixels(size, q);
		sw = size.width();
		sh = size.height();
    }
    ...
    if (desktop && !q->testAttribute(Qt::WA_DontShowOnScreen)) {                                // desktop widget
		...
		data.crect = QHighDpi::fromNativePixels(data.crect, (QWidget*)NULL);
    }
...
//         }
        ...
        if (isVisibleOnScreen && AdjustWindowRectEx(&rect, style & ~WS_OVERLAPPED, FALSE, exsty)) {
			margins = QMargins(rect.left, rect.top, rect.right, rect.bottom);
			margins = QHighDpi::fromNativePixels(margins, q);

            QTLWExtra *td = maybeTopData();
            if (wasMoved && (td && !td->posFromMove)) {
                x = data.crect.x() + margins.left();
                y = data.crect.y() + margins.top();
            }

            if (q->testAttribute(Qt::WA_Resized)) {
                w = data.crect.width() + (margins.right() - margins.left());
                h = data.crect.height() + (margins.bottom() - margins.top());
            }
        }
        ...
        if (isVisibleOnScreen && topLevel && initializeWindow && (style & WS_POPUP)) {
            if (!q->testAttribute(Qt::WA_Resized)) {
                ...
                h = 4*sh/10;
                if (extra) {
                    int dx = margins.right() - margins.left();
                    int dy = margins.bottom() - margins.top();
		...
		QRect rc = QHighDpi::toNativePixels(QRect(x, y, w, h), q);	
		...
		QRect rc = data.crect;
		rc = QHighDpi::toNativePixels(rc, q);
		id = CreateWindowEx(exsty, reinterpret_cast<const wchar_t *>(windowClassName.utf16()),
			reinterpret_cast<const wchar_t *>(title.utf16()), style,
			rc.left(), rc.top(), rc.width(), rc.height(),
			parentw, NULL, appinst, NULL);
    ...
    if (desktop) {
        ...
        if (data.crect.width() == 0 || data.crect.height() == 0) {
            data.crect = QRect(QHighDpi::fromNativePixels(QPoint(pt.x, pt.y), q), data.crect.size());
        } else {
			//modify caizhiru
            data.crect = QRect(QPoint(pt.x, pt.y),
                               QPoint(pt.x + cr.right - 1, pt.y + cr.bottom - 1));
			data.crect = QHighDpi::fromNativePixels(data.crect, q);
        }
...
QPoint QWidget::mapToGlobal(const QPoint &pos) const
{
    ...
    if (!isVisible() || parentWindow->isMinimized() || !testAttribute(Qt::WA_WState_Created) || !internalWinId()
        ...
        if (extra && extra->topextra && extra->topextra->embedded) {
			...
			pt = QHighDpi::toNativePixels(pt, this);
            POINT p = {pt.x(), pt.y()};
            ClientToScreen(parentWindow->effectiveWinId(), &p);
            return  QHighDpi::fromNativePixels(QPoint(p.x, p.y), this);
        } else {
	...
	tmp = QHighDpi::toNativePixels(tmp, this);
    p.x = tmp.x();
    p.y = tmp.y();
...
QPoint QWidget::mapFromGlobal(const QPoint &pos) const
{
    ...
    QWidget *parentWindow = window();
    QWExtra *extra = parentWindow->d_func()->extra;
	QPoint nativePos = QHighDpi::toNativePixels(pos, parentWindow);
    ...
    if (!isVisible() || parentWindow->isMinimized() || !testAttribute(Qt::WA_WState_Created) || !internalWinId()
        ...
        if (extra && extra->topextra && extra->topextra->embedded) {
            POINT p = { nativePos.x(), nativePos.y()};
            ScreenToClient(parentWindow->effectiveWinId(), &p);
            return mapFrom(parentWindow, QHighDpi::fromNativePixels(QPoint(p.x, p.y), this));
        } else {
    ...
    return d->mapFromWS(QHighDpi::fromNativePixels(QPoint(p.x, p.y), this));
    ...
    if (isWindow()) {
		...
		if (ulong(d->topData()->savedFlags) & WS_SYSMENU)
		    style |= WS_SYSMENU;
                ...
                if (newstate & Qt::WindowActive)
                    swpf |= SWP_NOACTIVATE;
				QRect native_r = QHighDpi::toNativePixels(r, this);
                SetWindowPos(internalWinId(), HWND_TOP, native_r.left(), native_r.top(), native_r.width(), native_r.height(), swpf);
                d->updateFrameStrut();
            } else {
...
void QWidgetPrivate::setWSGeometry(bool dontShow, const QRect &)
{
    ...
    if (parentWRect.isValid()) {
        ...
        if (data.wrect.isValid() && QRect(QPoint(),data.crect.size()).contains(data.wrect)) {
            ...
            if (data.wrect.contains(vrect)) {
				...
				{
					//modify caizhiru
					QRect rc = QHighDpi::toNativePixels(xrect, q);
					MoveWindow(q->internalWinId(), rc.x(), rc.y(), rc.width(), rc.height(), true);
    ...
    if (q->internalWinId()) {
		...
		//modify caizhiru
		QRect rc = QHighDpi::toNativePixels(xrect, q);
		MoveWindow(q->internalWinId(), rc.x(), rc.y(), rc.width(), rc.height(), !jump);
    ...
    if (q->testAttribute(Qt::WA_WState_ConfigPending)) {        // processing config event
        ...
        if (q->windowType() == Qt::Desktop) {
            ...
            if (extra) {
				...
				rc_.setRight((x + w - 1));
				rc_.setBottom((y + h - 1));
				rc_ = QHighDpi::toNativePixels(rc_, q);

                fs.setLeft(rc_.left() - fs.left());
                fs.setTop(rc_.top() - fs.top());
                fs.setRight(rc_.right() + fs.right());
                fs.setBottom(rc_.bottom() + fs.bottom());
            }
            if (w == 0 || h == 0) {
                q->setAttribute(Qt::WA_OutsideWSRange, true);
               ...
               // RECT rect;
                ...
                if (isTranslucentWindow) {
					...
					RECT rect = {0};
					GetClientRect(q->internalWinId(), &rect);
					QSize clientSize = QSize(rect.right - rect.left, rect.bottom - rect.top);
					clientSize = QHighDpi::fromNativePixels(clientSize, q_func());
					...
					QPoint clientPos = QPoint(rcNormalPosition.left, rcNormalPosition.top);
					clientPos = QHighDpi::fromNativePixels(clientPos, q_func());
					data.crect.setRect(clientPos.x() + fStrut.left(),
						clientPos.y()+ fStrut.top(),
						clientSize.width(),
						clientSize.height());
...
{
    Q_D(const QWidget);
	QWindowsScreen* screen = NULL;
	QWidget *topLevel = window();
	if (topLevel) {
		QApplicationPrivate::platform_integration->screen(topLevel);
	}
	if (!screen)
		screen = QApplicationPrivate::platform_integration->primaryScreen();
	...
	if (m == PdmWidth) {
		...
		return topLevel ? QHighDpiScaling::factor(topLevel) : QHighDpiScaling::factor(screen);
	}
	else if (m == PdmDevicePixelRatioScaled) {
		return (QPaintDevice::devicePixelRatioFScale() * (topLevel ? QHighDpiScaling::factor(topLevel) : QHighDpiScaling::factor(screen)));
	}
	else {
...
void QWidgetPrivate::registerDropSite(bool on)
{
    ...
    if (on) {
        ...
        createExtra();
#ifndef QT_NO_DRAGANDDROP
        if (!q->internalWinId() && q->nativeParentWidget())
...
void QWidgetPrivate::setMask_sys(const QRegion &region)
{
	...
	QRegion native_region = QHighDpi::toNativeLocalRegion(region, q);
    ...
    CombineRgn(wr, native_region.handle(), 0, RGN_COPY);
