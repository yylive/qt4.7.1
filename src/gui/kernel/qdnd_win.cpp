                                        ...
                                        POINTL pt, LPDWORD pdwEffect)
{
	...
	const QPoint logic_pt = QHighDpi::fromNativePixels(QPoint(pt.x,pt.y), dragEnterWidget);
    lastPoint = dragEnterWidget->mapFromGlobal(logic_pt);
...
QOleDropTarget::DragOver(DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect)
{
...
#endif
	const QPoint logic_pt = QHighDpi::fromNativePixels(QPoint(pt.x, pt.y), widget);
    QWidget *dragOverWidget = widget->childAt(widget->mapFromGlobal(logic_pt));
    if (!dragOverWidget)
    ...
    QPoint tmpPoint = dragOverWidget->mapFromGlobal(logic_pt);
    ...
    if (!dragOverWidget->internalWinId() && dragOverWidget != currentWidget) {
        ...
        if (!dragOverWidgetGuard) {
            dragOverWidget = widget->childAt(widget->mapFromGlobal(logic_pt));
            if (!dragOverWidget)
                dragOverWidget = widget;
        }
...
QOleDropTarget::Drop(LPDATAOBJECT /*pDataObj*/, DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect)
{
...
#endif
	const QPoint logic_pt = QHighDpi::fromNativePixels(QPoint(pt.x, pt.y), widget);
    QWidget *dropWidget = widget->childAt(widget->mapFromGlobal(logic_pt));
    if (!dropWidget)
        dropWidget = widget;
    ...
    lastPoint = dropWidget->mapFromGlobal(logic_pt);
