...
*/
#ifndef QT_NO_WHEELEVENT
class QAbstractScrollAreaWheelEvent : public QWheelEvent
{
public:
	explicit QAbstractScrollAreaWheelEvent(const QPoint &pos, QWheelEvent *e);
	virtual ~QAbstractScrollAreaWheelEvent();

protected:
	QWheelEvent *relate;
};

QAbstractScrollAreaWheelEvent::QAbstractScrollAreaWheelEvent(const QPoint &pos, QWheelEvent *e)
	: QWheelEvent(*e)
	, relate(e)
{
	p = pos;
...
QAbstractScrollAreaWheelEvent::~QAbstractScrollAreaWheelEvent()
{
	if (isAccepted())
		relate->accept();
	else
...
{
    Q_D(QAbstractScrollArea);
	QScrollBar *scrollBar = e->orientation() == Qt::Horizontal ? d->hbar : d->vbar;
	QAbstractScrollAreaWheelEvent we(scrollBar->mapFrom(this, e->pos()), e);
	QApplication::sendEvent(scrollBar, &we);
}
#endif
