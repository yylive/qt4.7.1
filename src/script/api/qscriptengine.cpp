	  ...
	  m_globalObject(NULL)
{
    ...
    m_globalObject = new (globalData)QScript::GlobalObject();
	  JSC::JSGlobalObject *globalObject = m_globalObject;
...
void QScriptEngine::startProfiling(const QString &title)
{
	Q_D(QScriptEngine);
	d->m_globalObject->needProfiling = true;
	d->globalExec()->lexicalGlobalObject()->setProfileGroup((unsigned)this);
	JSC::Profiler::profiler()->startProfiling(d->globalExec(), title.toUtf8().constData());
}


typedef pair<JSC::UString::Rep*, unsigned> NameCountPair;
...
static QString parseNode(JSC::ProfileNode *node, double sampleCount) {
	QString name = QString::fromUtf8(node->functionName().UTF8String().c_str());
	QString url = QString::fromUtf8(node->url().UTF8String().c_str());
	return QString("{\n\"totalTime\":%1,\n\"numberOfCalls\":%2,\n\"functionName\":\"%3\",\n\"url\":\"%4\",\n\"lineNumber\":%5,\n\"callUID\":%6,\n\"children\":[\n")
		.arg(sampleCount ? sampleCount : 1)
		.arg(node->numberOfCalls())
		.arg(name)
		.arg(url)
		.arg(node->lineNumber())
		.arg(node->callIdentifier().hash());
}
static double profileNodeJSON(JSC::ProfileNode *node, QIODevice *file)
{
    // Print function names
    double sampleCount = node->actualTotalTime() * 1000;
		file->write(parseNode(node, sampleCount).toUtf8().constData());

    // Print children's names and information
    double sumOfChildrensCount = 0.0;
		JSC::StackIterator currentChild = node->children().begin();
    while(currentChild != node->children().end()) {
        sumOfChildrensCount += profileNodeJSON((*currentChild).get(), file);
				if(++currentChild != node->children().end()) {
					file->write(",");
				} else {
					break;
				}
    ...
    // Print remainder of samples to match sample's output
		file->write(QString("\"selfTime\":%1\n").arg(sampleCount - sumOfChildrensCount).toUtf8().constData());
		file->write("}\n");

    return node->actualTotalTime();
}

static void profileJSON(JSC::Profile *profile, const QString &uid, QIODevice *file)
{
		file->write("{\n");
		file->write(QString("\"title\":\"%1\",\n\"uid\":\"%2\",\"typeId\":\"CPU\",\n\"head\":")
				...
				.arg(uid)
				.toUtf8().constData());
    profileNodeJSON(profile->head(), file);
		file->write("}");
}

void QScriptEngine::stopProfiling(const QString &title, const QString &uid, QIODevice *file)
{
	Q_D(QScriptEngine);
	d->m_globalObject->needProfiling = false;
	WTF::RefPtr<JSC::Profile> pro = JSC::Profiler::profiler()->stopProfiling(d->globalExec(), title.toUtf8().constData());
	profileJSON(pro.get(), uid, file);
}
