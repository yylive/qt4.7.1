...
void VcxprojGenerator::initConfiguration()
{
    ...
    if (project->isEmpty("CE_SDK") || project->isEmpty("CE_ARCH")) {
        conf.Name += (conf.idl.TargetEnvironment == "x64" ? "|x64" : "|Win32");
    } else {
...
void VcxprojGenerator::initIDLTool()
{
	VCXConfiguration &conf = vcxProject.Configuration;
	if (conf.linker.TargetMachine == "MachineX64")
	{
		conf.idl.TargetEnvironment = "x64";
	}
}
